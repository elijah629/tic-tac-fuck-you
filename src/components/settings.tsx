...
import { AudioLinesIcon, LucideChevronLeft, LucideChevronRight } from "lucide-react";
...
export function Settings() {
  ...
  return <Popover onOpenChange={setOpen} open={open}>
      <PopoverTrigger asChild>
        <Button className="fixed bottom-10 right-10 z-20" size="icon" variant="outline">
...
<Button size="icon" disabled={soundtrackId === 0} onClick={() => {
              setTrack(soundtrackId - 1);
          }}>
              <LucideChevronLeft size={24}/>
            </Button>
            <div className="bg-secondary rounded-md px-4 py-2 flex flex-col">
               <Link href={track.source} className="underline">{track.name}</Link>
               <span className="text-sm">By <Link href={track.author.link} className="underline">{track.author.name}</Link></span>
            </div>
<Button size="icon" disabled={soundtrackId === SOUNDTRACKS.length - 1} onClick={() => {
              setTrack(soundtrackId + 1);
          }}>
        ...
        <div className="grid gap-2">
            ...
            <Slider id="master" value={[volume.master * 100]} onValueChange={([v]) => {
...
setVolume({ ...volume, soundtrack: v / 100 })
          ...
          <Button onClick={() => {
            setOpen(false);
        }} className="text-2xl">Close</Button>
        </div>
      </PopoverContent>
    </Popover>
}
